...
import { Employee, Shift, ShiftStatus, CompanyRole } from '../../../lib/types'
import { useAuth } from '../../../lib/contexts/AuthContext'
import { getEmployees } from '../../../lib/services/employees'
import { getShifts, saveShift, deleteShift } from '../../../lib/services/shifts'
...
export default function RosterPage() {
  ...
  const shifts = useRealtimeShifts(
    companyId,
    shiftsData,
    weekDates[0],
    weekDates[6]
  ...
  const fetchData = useCallback(async () => {
    ...
    // ✅ Nunca deixe tela branca: se não tiver company, finaliza loading e mostra estado
    if (!companyId) {
      setEmployees([])
      setShiftsData([])
      setIsLoading(false)
    ...
    try {
      const [emps, weekShifts] = await Promise.all([
        getEmployees(companyId),
        getShifts(companyId, weekDates[0], weekDates[6]),
      ])

      setEmployees(Array.isArray(emps) ? emps : [])
      setShiftsData(Array.isArray(weekShifts) ? weekShifts : [])
    } catch (e: any) {
      console.error('[roster] fetchData error:', e)
      setEmployees([])
      setShiftsData([])
      setLoadError(e?.message ?? 'Failed to load roster data')
    } finally {
      setIsLoading(false)
    }
  }, [companyId, weekDates])
  ...
  useEffect(() => {
    ...
    // só busca quando já sabemos se tem user/companyId
    fetchData()
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <div className="max-w-lg w-full bg-white border border-slate-200 rounded-lg p-6">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Your account ({user.email}) isn’t linked to a company yet, so we can’t
            load employees or shifts.
          </p>
          ...
          <p className="mt-4 text-xs text-slate-500">
            Fix: set <code className="px-1 py-0.5 bg-slate-100 rounded">company_id</code> for
            this user in <code className="px-1 py-0.5 bg-slate-100 rounded">public.users</code>.
          </p>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <div className="max-w-lg w-full bg-white border border-red-200 rounded-lg p-6">
          <h1 className="text-lg font-semibold text-slate-900">Failed to load</h1>
              ...
              <thead className="bg-slate-50 sticky top-0 z-20">
                <tr>
                  ...
                  {weekDates.map((date, i) => (
                    <th
                      key={date}
                      className="px-2 py-3 text-center text-xs font-medium text-slate-500 uppercase tracking-wider min-w-[140px] border-b border-r border-slate-100 bg-slate-50"
                    >
                  ...
                  <tr key={emp.id}>
                    ...
                    {weekDates.map((date) => {
                      ...
                      return (
                        <td
                          key={date}
                          className="px-1 py-2 align-top border-r border-slate-100 h-24 min-h-[6rem] relative group hover:bg-slate-50 transition-colors"
                        >
                          <div className="space-y-1">
                            {cellShifts.map((shift) => (
                              <button
                                ...
                                disabled={!isManager}
                                className={`w-full text-left p-2 rounded border text-xs shadow-sm transition-all ${
                                  isManager
                            ...
                            {isManager && (
                              <button
                                ...
                                className={`w-full py-1.5 text-xs border border-dashed border-slate-200 rounded text-slate-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all flex items-center justify-center ${
                                  cellShifts.length > 0
      ...
      >
        <form onSubmit={handleSaveShift} className="space-y-4 text-left">
          ...
          <Input
            label="Role (Optional)"
            name="role"
            defaultValue={editingShift?.role || ''}
            placeholder="Override default role"
          />
          ...
          <div className="pt-6 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
            {editingShift ? (
              <button
                ...
                className="w-full sm:w-auto px-4 py-2 text-red-600 hover:bg-red-50 rounded-md text-sm font-medium flex items-center justify-center transition-colors"
              >
